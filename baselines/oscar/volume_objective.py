...
class VolumeObjective:
    ...
    def _features(self, x: torch.Tensor) -> torch.Tensor:
        ...
        """
        phi = self.clip.encode_image_from_pixels(x, size=self.cfg.clip_image_size)
    ...
    def volume_loss_and_grad(self, x_in: torch.Tensor):
        ...
        with torch.no_grad(), torch.amp.autocast("cuda", enabled=use_amp):
            ...
            for s in range(0, B, chunk):
                phi_list.append(self._features(x_all[s:s+chunk]))
            phi_all = torch.cat(phi_list, dim=0)

        with torch.enable_grad():
            phi_leaf = phi_all.detach().to(torch.float32).clone().requires_grad_(True)

            K = self._kernel(phi_leaf)
        ...
        for s in range(0, B, chunk):
            ...
            with torch.enable_grad():
                phi_chunk = self._features(x_chunk)
